            ...
            class ctrCalculator {
                ...
                checkNum(value) {
                    ...
                    if (isNaN(number) || number < 0 || number > 1000) {
                        alert('Each number should be within the range 0-1000');
                        return false;
                    }
                    return true;
                }
                ...
                checkOperator(value) {
                    ...
                    // Implement the logic to check for errors in the expression            
                    if (operators.length > 2) {
                        document.getElementById('display').value = 'Error: Only max 3 numbers are allowed in one expression';
                        return false;
                    }

                    // Check if all operators are of the same type
                    const operatorType = calculatorUI.getOperatorType(operators[0]);
                    for (let i = 1; i < operators.length; i++) {
                        if (calculatorUI.getOperatorType(operators[i]) !== operatorType) {
                            document.getElementById('display').value = 'Error: Only the same type of operator allowed in one expression';
                            return false;
                        }
                    } 
                    ...
                    if (operators.length === 1 && operators[0] === '*') {
                        ...
                        if (!(num1 >= 0 && num1 <= 9 && num2 >= 0 && num2 <= 10)) {
                            alert("Error: Multiplication operands must satisfy the constraints where 0 <= a <= 9 and 0 <= b <= 10 ");
                    ...
                    if (operators.length === 1 && operators[0] === '/') {
                        ...
                        if (!(num1 >= 10 && num1 <= 99 && num2 >= 1 && num2 <= 10)) {
                             alert("Error: Division operands must satisfy the constraints where 10 <= a <= 99 and  1 <= b <= 10 ");
                ...
                calculate() {
                    ...
                    try {
                        ...
                        if (expression.includes('/') && !Number.isInteger(result)) {
                            ...
                            calculatorUI.displayError('Error: Result out of range (0-1000)');
                        }
                    } catch (error) {
                        // Display an error message using CalculatorUI method
            ...
            class CalculatorUI {
                ...
                enterInputNumber(value) {
                    ...
                    if (this.calculator.checkNum(value)) {
                        ...
                        // Display alert if the input number is greater than 1000
                        if (parseFloat(newValue) > 1000) {
                            alert('Number range is 0-1000');
                            return;
                        }
                ...
                getOperatorType(operator) {
                    // Define the allowed operator types (you can customize this based on your requirements)
                    ...
                    // Get the operator type for the provided operator
                    return allowedOperatorTypes[operator];
                }

                displayResult(result) {
                    // Display the result in the calculator display
                    document.getElementById('display').value = parseFloat(result.toFixed(1));
                }
